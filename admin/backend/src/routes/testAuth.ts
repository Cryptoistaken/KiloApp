...
import { cookie, isAdmin, signSession } from "../lib/session";
...
testAuth.post("/login", async (c) => {
  ...
  // Admin-only website: the test backdoor mints admin sessions only.
  if (!isAdmin(c.env, uid)) return c.json({ error: "Admin access only", code: "admin_only" }, 403);
