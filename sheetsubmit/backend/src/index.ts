...
app.post("/api/auth/telegram/verify", async (c) => {
  ...
  // Admin-only website: regular users cannot sign in here at all (neither as
  // user nor as admin). The Android app serves users; this site serves admins.
  if (!isAdmin(c.env, claims.uid)) return c.json({ ok: false, error: "Admin access only", code: "admin_only" }, 403);
