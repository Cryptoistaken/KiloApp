...
| `sheet-bubble-doodle.html` | Minimal doodle of the wanted popup (header actions + check menu + compact grid + auto-scroll) |
| `sheet-bubble.html` | Standalone interactive HTML reference for the native Circle Sheet bubble: Cookie/2fa/Page modes, clipboard capture, Skip 2FA, 230dp popup on the proxy/SMS shell, scrim/X close |
...
| `FloatingControlService.kt` | Floating bubble (60dp) + flag pill overlays, long-press popup; WindowManager, SYSTEM_ALERT_WINDOW. Circle style shares lock visuals; long-press opens CircleBubbleMenu (Proxy tap toggles, Proxy long-press opens country menu, SMS provisions+copies, Name copies, Sheet opens the configured file popup, or toasts + opens the Sheet tab when none is selected) |
...
| `CircleBubbleMenu.kt` | Circle-menu overlay: Proxy/SMS/Sheet/Name bubbles around the anchor, alignment + size from prefs, scrim dismiss; Sheet bubble shows the configured file's preset icon (generic glyph when none), long press applies Skip 2FA |
| `SheetMenuOverlay.kt` | Native 230dp x 280dp Sheet file popup on the exact proxy/SMS shell (44dp bar with file identity + compact undo/redo (18dp, 6dp apart, scaled-down small drawables so full-bleed arrowheads don't clip) + Check split button (10sp, 10dp spinner, existing dropdown replaced by `ic_ss_check_arrow_triangle` (solid down-triangle). Vector intrinsic size is 10dp (not 24) on purpose: ImageButton measures to `max(drawableIntrinsic + padding, minWidth)`, so a 24dp intrinsic overrides the 18dp button and renders the icon full-size; 10dp intrinsic + 4dp padding keeps the button at 18dp and renders the caret at 10dp (centerInside, never upscales). Fill #000000, tinted per-theme by the existing setColorFilter (white/black). In-app keeps the original chevron.) with floating top-end dropdown over the grid (app-style overlay, never shifts layout; no divider — grid sticky header is the separation) — no close/"..." buttons, scrim-tap dismiss, smart placement, grow-in with a hardware layer so the live grid doesn't re-composite per frame): in-app Check split button (one joined pill, spinner + "Checking" state, UID/Simple/Advanced MiniSwitch menu on the same prefs/exclusivity as the app; Check runs file-scoped in the popup honoring the toggles, never opens the app) + the shared SheetGrid hosted read-only in a ComposeView at compact metrics (24dp rows/rails, 10sp cells) — literally the same file view; the popup renders a 20-row window at first paint that grows +20 as data is saved and when scrolled near its end (so small files open cheap while history stays reachable) (direct foundation dep; view-tree owners attach by reflection on the window root — ComposeView resolves upward — since the overlay compile classpath lacks lifecycle-runtime/savedstate), smart scroll targets the last data row and only recenters when it is not already visible (fits-on-screen files show row 1, no jump; overflowing lists reveal newly-saved rows; sticky-header index offset applied), clipboard auto-captured into the active row on open, fresh cookies auto-run the whole-file UID check until the UID toggle is off; open pipeline paints the loaded grid first, then capture + auto-check update in place |
...
| `sheet/SheetBubbleCoordinator.kt` | File-scoped Sheet bubble snapshot/atomic row writes + single-step DB undo-redo (newest entry, matching pop-newest) + toggle-honoring whole-file check (UID liveness + PAGE Simple/Advanced sweeps, one undo entry) + grow-5-when-4-trailing-empty-remain; never publishes global open-file state |
...
- `screens/sheet/` — Sheet tab port (user-only, same for admins in-app; no admin views): SheetScreen (My Files / Wallet / Archive tabs + open-file routing + BackHandler), SheetUi (site-exact status colors, StatusDot, PresetIcon, PasswordBadge, empty state, shared styleKey/parseHexColor), SheetGrid (the file grid shared by the editor and the floating bubble: sticky header, cells, dots; interactions + cell/dot/footer slots, null = static read-only), SheetFilesTab + SheetFileCard + SheetCreateDialogs (cards, FAB create, type/password picks, rename, SAF xlsx download, share-a-copy xlsx via system sheet, dep-free xlsx import; cards expose Use with bubble selection with a green Bubble badge + Remove from bubble on the active file), SheetArchiveTab (restore/delete-forever, days-left), SheetWalletTab (balance USD/BDT, withdraw form, history), SheetDetailScreen (toolbar, grid via shared SheetGrid, QuickEditBar, selection bar, archived viewer)
...
- `layout/` — `app_item.xml`, `bubble_menu.xml` (country popup), `bubble_sheet_menu.xml` (230dp x 280dp Sheet file window on the proxy/SMS shell), `bubble_country_row.xml`, `notification_action.xml` (RemoteViews layout for the notification Connect/Disconnect pill)
