...
| `worker/` | Backend API (Hono, copied from standalone SheetSubmit `backend/`). Two entries: `src/server.ts` (Bun/Railway, all reads+writes) + `src/entry.ts` (Cloudflare Workers `kiloapi`, stage 1: sms `/v1/*` + `/v1/check` + safe reads via Neon HTTP; writes stay on Railway). Deploy edge: `bun run edge:dry` / `edge:deploy` in `worker/`; secrets via `wrangler secret put` (see `wrangler.toml`). |
