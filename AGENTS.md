...
| `sheet/SheetBubbleCoordinator.kt` | File-scoped Sheet bubble snapshot/atomic row writes + single-step DB undo-redo (newest entry, matching pop-newest) + whole-file UID check + grow-5-when-4-trailing-empty-remain; never publishes global open-file state |
